...
class robot:
    ...
    def cte(self, radius):
        cte = 0.0
        if(self.x > radius and self.x < 3.0*radius):        # in straight section
            if(self.y > radius):                   # Top part going right
                cte = self.y-radius*2.0
            else:
                cte = -self.y
        elif(self.x < radius):                       #In the left half-circle
            # m = (self.y - radius) / (self.x - radius)
            # a = 1 + m ** 2
            # b = -(radius ** 2) * (2.0 + 2.0 * (m ** 2))
            # c = (radius ** 2) * (m ** 2)
            # x = (-b - sqrt(b ** 2 - 4.0 * a * c)) / 2 * a
            # a2 = 1
            # b2 = -2.0*radius
            # c2 = (x - radius) ** 2
            ...
            # else:
            #     y = (-b2 + sqrt(b2 ** 2 - 4 * a2 * c2)) / 2 * a2
            # cte = sqrt((x - self.x) ** 2 + (y - self.y) ** 2)
            # if((self.x-radius)**2 + (self.y-radius)**2 < radius**2):
            #     cte = -cte
            # print 'My cte = ', cte
            cte = sqrt((self.x - radius) ** 2 + (self.y - radius) ** 2) - radius
            print 'Their cte = ', cte
        elif(self.x > radius*3):                     # In the right half-circle
            # m = (self.y - radius) / (self.x - radius*3)
            # a = 1 + m**2
            # b = -(radius**2)*(6+6*(m**2))
            # c = (radius**2)*(9*(m**2)+8)
            ...
            #     y = (-b2 + sqrt(b2 ** 2 - 4 * a2 * c2)) / 2 * a2
            # cte = sqrt((x - self.x) ** 2 + (y - self.y) ** 2)
            # if ((self.x - 3*radius) ** 2 + (self.y - radius) ** 2 < radius ** 2):
            #     cte = -cte
            # print 'My cte = ', cte
...
def run(params, radius, printflag=False):
    ...
    for i in range(N * 2):
        ...
        print crosstrack_error, diff_crosstrack_error, int_crosstrack_error
