...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        beta = d / L * tan(steering)
        new_position = robot(self.length)
        if (beta < 0.001):
            new_x = x + (d * cos(teta))
            new_y = y + (d * sin(teta))
            new_teta = (teta + beta) % (2 * pi)
            new_position.set(new_x, new_y, new_teta)
        else:
            radius = d / beta
            cx = x - sin(teta) * radius
            cy = y + cos(teta) * radius
            new_x = cx + sin(teta + beta) * radius
            new_y = cy - cos(teta + beta) * radius
            new_teta = (teta + beta) % (2 * pi)
            new_position.set(new_x, new_y, new_teta)

        new_position.set_noise(self.bearing_noise, self.steering_noise, self.distance_noise)

        return new_position  # make sure your move function returns an instance
