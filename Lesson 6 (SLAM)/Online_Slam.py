...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    size = (num_landmarks + 1) * 2      #retain all landmarks but only most recent pose
    ...
    # I'm assuming an initial postion at (50,50)
    Omega.value[0][0] += 1
    Omega.value[1][1] += 1
    ...
    for i in range(len(data)):
        x = 0
        y = x + 1
        Z = data[i][0]          #measurements data
        M = data[i][1]          #motion data

        #sense
        for k in range(len(Z)):
            landmarkIndex = 2*(Z[k][0]+1)


            # for x
            Omega.value[x][x] += 1 / measurement_noise
            Omega.value[x][landmarkIndex] += -1 / measurement_noise
            Omega.value[landmarkIndex][x] += -1 / measurement_noise
            Omega.value[landmarkIndex][landmarkIndex] += 1 / measurement_noise

            Xi.value[x][0] += -Z[k][1] * (1 / measurement_noise)
            Xi.value[landmarkIndex][0] += Z[k][1] * (1 / measurement_noise)

            # for y
            Omega.value[y][y] += 1 / measurement_noise
            Omega.value[y][landmarkIndex+1] += -1 / measurement_noise
            Omega.value[landmarkIndex+1][y] += -1 / measurement_noise
            Omega.value[landmarkIndex+1][landmarkIndex+1] += 1 / measurement_noise

            Xi.value[y][0] += -Z[k][2] * (1 / measurement_noise)
            Xi.value[landmarkIndex+1][0] += Z[k][2] * (1 / measurement_noise)

        #Motion
        nextX = x+2
        nextY = y+2

        # expand matrix for new position
        auxList = range(4, size+2)          #list to map all new landmark indexes
        Omega = Omega.expand(size+2,size+2, [0, 1] + auxList, [0, 1] + auxList)
        Xi = Xi.expand(size+2, 1, [0, 1] + auxList, [0])


        #for x
        Omega.value[x][x] += (1 / motion_noise)
        Omega.value[x][nextX] += -(1 / motion_noise)
        Omega.value[nextX][x] += -(1 / motion_noise)
        Omega.value[nextX][nextX] += (1 / motion_noise)

        Xi.value[x][0] += -M[0] * (1 / motion_noise)
        Xi.value[nextX][0] += M[0] * (1 / motion_noise)

        #for y
        Omega.value[y][y] += (1 / motion_noise)
        Omega.value[y][nextY] += -(1 / motion_noise)
        Omega.value[nextY][y] += -(1 / motion_noise)
        Omega.value[nextY][nextY] += (1 / motion_noise)

        Xi.value[y][0] += -M[1] * (1 / motion_noise)
        Xi.value[nextY][0] += M[1] * (1 / motion_noise)

        A = Omega.take([0, 1], [2, 3] + auxList)
        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])

        Omega_prime = Omega.take([2, 3] + auxList, [2, 3] + auxList)
        Xi_prime = Xi.take([2, 3] + auxList, [0])
